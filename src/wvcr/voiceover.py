...
def save_pcm_to_wav(
    pcm_data, output_file, sample_rate=24000, channels=1, sample_width=2
):
...
def write_audio(
    ...
    try:
        ...
        create_params = {"input": text, "response_format": "pcm"}
...
"""
        ...
        with config.client.audio.speech.with_streaming_response.create(
            **create_params
            ...
            _stream_audio_with_buffer(
                response, stream, pcm_data, stop_event, buffer_size
            )
...
def voiceover_clipboard(
    ...
    if notifier is None:
        notifier = SystemNotificationManager()
    ...
    try:
        ...
        if play:
            ...
            key_monitor = create_key_monitor(
                audio_config.STOP_KEY, stop_callback, prefer_evdev=use_evdev
            )
            key_monitor.start()
            ...
            play_thread = threading.Thread(
                target=play_audio,
                args=(text, config, output_file, stop_playback, use_gpt_tts),
            )
            play_thread.start()
