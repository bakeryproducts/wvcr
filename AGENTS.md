...
- `llm.py` sends the MP3 + a prompt to `gemini-3.5-flash` with Google Search grounding enabled (`types.Tool(google_search=...)`), forcing plain-text Russian output. Prompt is anchored on the last moment of audio (not earlier topics) and always surfaces useful info even without an explicit question.
- `runner.py` wires hotkey → buffer snapshot → LLM call (own thread, non-blocking, drops overlapping presses) → `LayerShellNotificationManager` popup.
